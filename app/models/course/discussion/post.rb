...
class Course::Discussion::Post < ApplicationRecord
  ...
  # @!method self.include_drafts_for_managers(current_user)
  #   Includes draft posts if the user is the course manager/owner.
  #
  #   @param [User] current_user The user to determine access for.
  scope :include_drafts_for_managers, (lambda do |current_course_user, current_course|
    if current_course_user&.manager_or_owner? && current_course.component_enabled?(Course::RagWiseComponent)
      all
    else
