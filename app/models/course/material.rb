...
class Course::Material < ApplicationRecord
  ...
  #
  # @return [upPathname] The path of the material
  def path
    folder.path + name
  end
  ...
  def text_chunking!
    ...
    Course::Material::TextChunkJob.perform_later(self).tap do |job|
  ...
  def build_text_chunks
    ...
    begin
      ...
      File.open(attachment.path, 'r:ASCII-8BIT') do |file|
        ...
        chunks = chunking_service.file_chunking(false)
        embeddings = llm_service.generate_embeddings_from_chunks(chunks)
        chunks.each_with_index do |chunk, index|
          text_chunks.build(embedding: embeddings[index], content: chunk, chunker_id: User.system,
                            course_id: course_id)
        end
      end
