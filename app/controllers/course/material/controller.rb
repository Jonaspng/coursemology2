...
class Course::Material::Controller < Course::ComponentController
  ...
  def destroy_text_chunks
    materials = Course::Material.includes(:text_chunk_references).where(id: material_chunking_params[:material_ids])
    if Course::Material.destroy_text_chunk_references(materials)
