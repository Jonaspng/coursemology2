...
class Course::Admin::RagWiseSettingsController < Course::Admin::Controller
  ...
  def forums
    @forums = Course::Forum.where(course_id: @parent_courses.map(&:id)).
              includes(:course_forum_imports).map do |forum|
      {
        forum: forum,
  ...
  def set_parent_courses
    ...
    while (next_course = course.duplication_traceable&.source)
    ...
    @forum_imports = current_course.forum_imports
  end
